...
# SB-63: -LongForm switch on -Format StudyGuide (parametric long-form pacing).
# Generated by `Format-SBAutobookSSML -Format StudyGuide -LongForm -Topic 'Test Topic'
# -Date '2026-04-09'` against the same 3-paragraph stub the existing _SAMPLE_* fixtures
# use, after merge of SecondBrain PR #25 (commit 481482d on origin/main).
# Voice routing reuses the StudyGuide trio (Steffan/Aria/Jenny). Pacing constants:
# 1000ms chapter silence, 700ms emphasis wrapping silence, rate -2 on intro AND outro,
# rate -1 on key insight / decision / pattern lines.
_SAMPLE_STUDYGUIDE_LONGFORM = (
    '<voice required="Name=Microsoft Steffan Online"><rate speed="-2">'
    "Welcome. We're spending an hour with Test Topic today. Find somewhere comfortable -- this one's worth your full attention."
    "</rate></voice>\n"
    '<voice required="Name=Microsoft Steffan Online">This is paragraph one about the main topic.</voice>\n'
    "\n"
    '<silence msec="700"/><voice required="Name=Microsoft Aria Online"><rate speed="-1">'
    "Key Takeaway: The most important thing to remember is focus."
    '</rate></voice><silence msec="700"/>\n'
    "\n"
    '<silence msec="1000"/><voice required="Name=Microsoft Steffan Online">'
    "Chapter 1: Getting Started</voice>\n"
    '<voice required="Name=Microsoft Steffan Online">Here is the first chapter content.</voice>\n'
    '<silence msec="700"/><voice required="Name=Microsoft Jenny Online"><rate speed="-1">'
    "Decision: We will proceed with the new approach."
    '</rate></voice><silence msec="700"/>\n'
    '<voice required="Name=Microsoft Jenny Online">Another regular line here.</voice>\n'
    '<silence msec="500"/><voice required="Name=Microsoft Jenny Online"><rate speed="-2">'
    "That's Test Topic. Let it sit for a while -- the pieces tend to assemble themselves on a walk, or after sleep."
    "</rate></voice>"
)
...
# ─────────────────────────────────────────────────────────────
#  SB-63: -LongForm switch contract for StudyGuide format
# ─────────────────────────────────────────────────────────────


class TestStudyGuideLongFormContract:
    """Validate -LongForm SSML output asserts long-form pacing + StudyGuide voice trio.

    The -LongForm switch on -Format StudyGuide adds: 1000ms chapter heading silence
    (vs default 500ms), 700ms emphasis wrapping silences (vs default 500ms), rate -1
    wrapping on key insight / decision / pattern lines, rate -2 wrapping on the outro
    (vs default no-rate emission). Voice routing is unchanged from default StudyGuide.
    Intro/outro prose templates are the Pair A "Settle in" / "Let it settle" variants
    selected at SB-63 ship time (committed in AutobookCmdlets.ps1 as named constants).

    SB-8 rule: this fixture must exist before -LongForm output reaches production
    listening use. Behavioral assertions over the snapshot, not pure equality.
    """

    def test_voice_routing_matches_studyguide_trio(self):
        """Voice routing under -LongForm reuses the StudyGuide trio (Steffan/Aria/Jenny)."""
        voice_names = re.findall(r'<voice\s+required="Name=([^"]+)">', _SAMPLE_STUDYGUIDE_LONGFORM)
        assert voice_names, "No voice tags found in -LongForm output"
        for name in voice_names:
            assert name in (
                "Microsoft Steffan Online",
                "Microsoft Aria Online",
                "Microsoft Jenny Online",
            ), f"Unexpected voice in -LongForm output: {name}"

    def test_intro_wraps_in_rate_minus_2(self):
        """Intro line (first content line) wraps in <rate speed="-2"> per long-form pacing."""
        first_line = _SAMPLE_STUDYGUIDE_LONGFORM.splitlines()[0]
        assert '<rate speed="-2">' in first_line
        assert '</rate>' in first_line

    def test_outro_wraps_in_rate_minus_2(self):
        """Outro line (last non-empty content) wraps in <rate speed="-2"> -- new under -LongForm."""
        non_empty = [ln for ln in _SAMPLE_STUDYGUIDE_LONGFORM.splitlines() if ln.strip()]
        last_line = non_empty[-1]
        assert '<rate speed="-2">' in last_line, (
            f"Outro should wrap in rate -2 under -LongForm: {last_line!r}"
        )

    def test_emphasis_lines_wrap_in_rate_minus_1(self):
        """Key Insight / Decision / Pattern lines wrap in <rate speed="-1"> under -LongForm."""
        # Both the takeaway (Aria) and decision (Jenny) lines should have rate -1 wrappers
        assert '<rate speed="-1">Key Takeaway' in _SAMPLE_STUDYGUIDE_LONGFORM
        assert '<rate speed="-1">Decision' in _SAMPLE_STUDYGUIDE_LONGFORM

    def test_chapter_silence_is_1000ms(self):
        """Chapter/section heading silences are 1000ms under -LongForm (vs default 500ms)."""
        # Chapter 1 heading should have a leading 1000ms silence
        assert '<silence msec="1000"/><voice required="Name=Microsoft Steffan Online">Chapter 1' in _SAMPLE_STUDYGUIDE_LONGFORM

    def test_emphasis_silence_is_700ms(self):
        """Key insight / decision wrapping silences are 700ms under -LongForm (vs default 500ms)."""
        # Both leading and trailing 700ms wrap each emphasis line
        emphasis_silence_count = _SAMPLE_STUDYGUIDE_LONGFORM.count('<silence msec="700"/>')
        # Two emphasis lines (Key Takeaway + Decision) × 2 wraps each = 4 occurrences minimum
        assert emphasis_silence_count >= 4, (
            f"Expected >= 4 occurrences of 700ms silence (wrapping pairs around emphasis lines); "
            f"got {emphasis_silence_count}"
        )

    def test_no_colon_syntax_pseudo_tags(self):
        """No legacy colon-syntax pseudo-tags ({{Voice:...}}, {{Rate:...}}) appear."""
        for tag in ("{{Voice:", "{{Rate:", "{{Silence:", "{{Pause:"):
            assert tag not in _SAMPLE_STUDYGUIDE_LONGFORM, (
                f"-LongForm output contains broken tag syntax: {tag}"
            )
    ...
    def test_no_standalone_control_tag_lines(self):
        ...
        for i, line in enumerate(_SAMPLE_STUDYGUIDE_LONGFORM.splitlines(), 1):
            if not line.strip():
                continue
            assert not bad_line_re.match(line), (
                f"Standalone control tag on line {i}: {line!r}"
    ...
    def test_approved_voices_only(self):
        """Voices restricted to approved list (no Zira, Hazel, David, etc.)."""
        for name in ("Zira", "Hazel", "David"):
            assert name not in _SAMPLE_STUDYGUIDE_LONGFORM, f"Found forbidden voice: {name}"
        voice_names = re.findall(r'<voice\s+required="Name=([^"]+)">', _SAMPLE_STUDYGUIDE_LONGFORM)
        for name in voice_names:
            assert name.endswith("Online"), (
                f"Voice name does not end with 'Online': {name}"
    ...
    def test_longform_intro_template_is_pair_a(self):
        """Intro prose is the selected Pair A 'Settle in' template, not the default StudyGuide intro."""
        assert "Welcome. We're spending an hour with Test Topic today" in _SAMPLE_STUDYGUIDE_LONGFORM
        assert "Find somewhere comfortable" in _SAMPLE_STUDYGUIDE_LONGFORM
        # Default StudyGuide intro must NOT appear under -LongForm
        assert "personalized study guide on Test Topic" not in _SAMPLE_STUDYGUIDE_LONGFORM

    def test_longform_outro_template_is_pair_a(self):
        """Outro prose is the selected Pair A 'Let it settle' template, not the default."""
        assert "That's Test Topic. Let it sit for a while" in _SAMPLE_STUDYGUIDE_LONGFORM
        assert "the pieces tend to assemble themselves on a walk, or after sleep" in _SAMPLE_STUDYGUIDE_LONGFORM
        # Default StudyGuide outro must NOT appear under -LongForm
        assert "Happy learning, and remember -- you've got this" not in _SAMPLE_STUDYGUIDE_LONGFORM

    def test_default_studyguide_unchanged_byte_identical(self):
        """Sanity check: default StudyGuide (without -LongForm) snapshot is unchanged.

        Catches accidental regressions where the -LongForm conditional bleeds into the
        default StudyGuide path. The existing _SAMPLE_STUDYGUIDE literal must still hold
        the original default pacing (500ms silences, no rate wrappers on emphasis or outro).
        """
        # Default StudyGuide chapter silence is 500ms (not 1000)
        assert '<silence msec="500"/><voice required="Name=Microsoft Steffan Online">' in _SAMPLE_STUDYGUIDE
        assert '<silence msec="1000"/>' not in _SAMPLE_STUDYGUIDE
        # Default StudyGuide outro has no rate wrapper
