...
**Date:** 2026-03-23
**Status:** Design approved, spec reviewed
...
- `--input` (required): Path to PDF or EPUB. Supports glob patterns via Python `glob.glob()` — if multiple matches, process first and warn on stderr. If zero matches, error. This avoids the recurring bug where Python doesn't expand Windows `*` globs.
...
- `--format` (optional): `json` (default) or `text` (human-readable summary for debugging). When `json`, all stdout is valid JSON. When `text`, stdout is human-readable.
...
**Page sampling:** Scan all pages except first 2 (cover/title). No back-matter exclusion — the detector finds ALL headings. Back-matter classification happens at the Claude prompt level. **Short PDF handling:** If total pages <= 3, scan ALL pages including cover/title.
...
3. Matches chapter pattern: `^(Chapter|Part|CHAPTER|PART)\s+\d+`, `^\d+\.\s+\w`, `^\d+\s+[A-Z]` (dot-less numbered chapters), `^[IVXLC]+\.\s+\w`, `^(Introduction|Conclusion|Preface|Foreword|Epilogue|Prologue|Appendix)`

**Noise filtering** — remove candidates that are:
- **Headers/footers:** Normalized text (case-insensitive, whitespace-collapsed) appearing at same y-coordinate (+-5pt) on >50% of scanned pages. Tracked via dict keyed by `(normalized_text, rounded_y)` with page count — checked in a single pass at the end.
- **Page numbers:** Purely numeric or Roman numeral strings with no other text
- **Too short/long:** <3 chars or >120 chars
- **Running headers:** All-caps text <= 5 words appearing on >30% of pages (covers multi-word running headers like "THE FOURTH GOSPEL AND THE LAND")
...
Single-signal candidates score ~0.5, multi-signal candidates ~0.8-0.99. Note: candidates with 3+ signals will cluster at 0.99 cap — this is acceptable since Claude uses confidence for ranking, not as a precise probability.
...
Deduplicate against NCX entries: an HTML heading matches an NCX entry if their normalized text (stripped of leading numbering, case-insensitive, whitespace-collapsed) matches. When matched, keep the NCX entry (higher confidence) and discard the HTML duplicate.

**EPUB level assignment:**
- Native `<h1>` → h1, `<h2>` → h2, `<h3>` → h3
- NCX entries: infer from nesting depth (depth 0 → h1, depth 1 → h2, depth 2 → h3)
...
- PDF with no text layer (image-only) → exit 0, valid JSON with empty `heading_candidates` and a `"warning"` field

## Component 2: `Get-ChapterStructure` Rewrite

### Parameter Changes

Add `$InputFile` parameter (path to the source PDF/EPUB) alongside the existing `$TextContent` parameter. `$InputFile` is required for running the font detector. If `$InputFile` is not provided (backward compatibility), skip font detection and use text-only sampling.
...
**Short book handling:** If total words < 9000, send the entire text as a single block with no zone labels.
...
### Level Scheme Mapping

**Critical:** The font detector and Claude prompt use string levels (`"h1"`, `"h2"`, `"h3"`), but the downstream consumer `apply_chapter_hints()` in `pdf_to_balabolka.py` expects integer levels (1, 2, 3). Since `pdf_to_balabolka.py` is NOT being modified, `Get-ChapterStructure` must convert before writing the hints JSON:

- `"h1"` → integer `1` (maps to `parts` list in `apply_chapter_hints`)
- `"h2"` → integer `2` (maps to `chapters` list in `apply_chapter_hints`)
- `"h3"` → integer `3` (currently no handler in `apply_chapter_hints` — see h3 section below)
...
Note: Claude prompt uses integer levels (1, 2, 3) matching the existing `apply_chapter_hints()` convention (1 = Parts, 2 = Chapters). No string-to-int conversion needed on Claude's response. The font detector's string levels (`h1`, `h2`, `h3`) are only used in the `=== FONT-DETECTED HEADING CANDIDATES ===` display section for Claude's reference, not parsed back.

The font candidate display section format:
```
...
Confirm which are real chapter/section headings, flag false positives, and note any chapters in the text samples not in this list.
  ...
  h1  p.5   [0.85] "Preface" (16pt bold)
  h1  p.15  [0.90] "1 Promised land in the Old Testament" (16pt bold)
  h2  p.22  [0.75] "The Abrahamic Covenant" (13pt bold)
  ...
```

### EPUB Early-Exit

This check lives in **`Convert-ToKindle`** (where `$htmlContent` is in scope), not inside `Get-ChapterStructure`:
...
if ($InputFile -match '\.epub$') {
    ...
    if ($nonBackMatter.Count -ge 5) {
        ...
        # Skip Get-ChapterStructure call
    }
}
```

Skip only if >= 5 existing `<h1>` tags that are NOT back-matter keywords. If all h1s are back matter, that's the signal real chapters are missing — proceed with Claude.
...
**This insertion happens in `Convert-ToKindle`** after `Get-ChapterStructure` returns the heading list, operating on the HTML content that `Convert-ToKindle` already has in scope.
...
3. If exact match fails, fuzzy: match first 5+ words of the heading
...
| PDF with no text layer | Exit 0, empty candidates with warning field |
| Book < 9000 words | Send entire text, skip zone sampling |
| PDF <= 3 pages | Scan all pages including cover/title |
...
| `module/EbookAutomation.psm1` | Rewrite `Get-ChapterStructure` — add `$InputFile` param, new sampling, font integration, prompt, level mapping |
| `module/EbookAutomation.psm1` | Modify `Convert-ToKindle` — EPUB early-exit check, per-heading HTML insertion, h3 Calibre flag |

## Files NOT Modified

- `classify_source.py` — untouched
- `pattern_db.py` — untouched
- `pdf_to_balabolka.py` — untouched (h3 gap in `apply_chapter_hints` is a known future item)
...
```
# PDF with uniform font (no heading candidates expected)
python tools/detect_headings_font.py --input "<path-to-uniform-font-pdf>"
-> Exit 0, empty heading_candidates array

# Very short PDF (< 4 pages — should scan all pages)
python tools/detect_headings_font.py --input "<path-to-short-pdf>"
-> Scans all pages, finds any headings present

# Image-only PDF (no text layer)
python tools/detect_headings_font.py --input "<path-to-image-only-pdf>"
-> Exit 0, empty candidates, warning field set

# EPUB with no NCX and no heading tags
python tools/detect_headings_font.py --input "<path-to-minimal-epub>"
-> Falls through all detection layers, returns empty candidates
```
