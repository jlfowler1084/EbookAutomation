...
- **R3.3.** Output validated server-side: format allowlist (EPUB-eligible for leafbind today; MOBI/KFX never), **30 MB raw cap** (corrected from brainstorm's 36 MB — see Key Technical Decisions), per-job_id binding (no separate output_id in Wave 1)
...
- **Resend attachment ceiling:** 40 MB post-base64 (~30 MB raw). Source: `https://resend.com/docs/dashboard/emails/attachments`.
...
- **30 MB raw attachment cap, not 36 MB.** Brainstorm said 36 MB; external research shows Resend's 40 MB is post-base64, so raw inflates ~33%, making 30 MB the actual binding ceiling. Plan uses 30 MB. *(corrects origin: R3.3)*
...
- **30 MB raw cap** → corrected from brainstorm's 36 MB (Key Technical Decisions)
...
3. Backend: read `job_store` for the job's `output_path` and format → resolve path and assert `is_relative_to(settings.temp_dir)` (P1-4 boundary check) → validate format ∈ `{"epub"}` allowlist → validate raw size ≤ 30 MB (R3.3)
...
- Size check: `raw_bytes ≤ 30 * 1024 * 1024` (30 MB).
...
**Test scenarios:**
- Happy path — Valid EPUB job, valid `@kindle.com` recipient, file < 30 MB → 200 `{"status": "sent"}`; Resend called once; idempotency row inserted
