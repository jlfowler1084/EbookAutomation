...
- **Resend domain + sender configuration**: provision the chosen From address (`kindle@leafbind.io`) in the Resend dashboard with a sending-access-only API key restricted to `leafbind.io`. Implementer verifies this exists before Unit 4 deploys.
- **Resend webhook endpoint registration**: register `https://api.leafbind.io/webhooks/resend` in the Resend dashboard and capture the Svix signing secret into `/opt/ebookautomation/.env` as `WEB_RESEND_WEBHOOK_SECRET` before Unit 10 deploys.
...
- **Exact From-address final string** — locked to `kindle@leafbind.io` during EB-330 Lane D. `send.leafbind.io` is Resend's MAIL FROM / bounce subdomain under the verified `leafbind.io` domain, not the proven sending identity.
...
- Happy path (Playwright) — Click Send-to-Kindle on EPUB row: form expands with From-address hint visible; enter `you@kindle.com`; click Send; assert "Sent to *you@kindle.com*. Arrives within a few minutes IF kindle@leafbind.io..." copy appears
...
- Modify: `deploy/CLOUDFLARE.md` (add the new path to the combined WAF rule expression; document the webhook URL needs to be registered in the Resend dashboard with the secret stored in `/opt/ebookautomation/.env`)
...
| Sender address (`kindle@leafbind.io`) isn't on Resend's verified-domain config | Deploy-side blocker: implementer verifies via Resend dashboard before Unit 4 ships. Add to deploy checklist |

## Documentation / Operational Notes

- **`.env.example`** (or equivalent) needs `WEB_RESEND_API_KEY=` and `WEB_SEND_TO_KINDLE_FROM=kindle@leafbind.io`. Document scope (sending-access restricted to `leafbind.io`).
...
- **Resend domain provisioning** is a manual prerequisite — implementer creates the `kindle@leafbind.io` sender + restricted API key in the Resend dashboard before Unit 4 deploy.
